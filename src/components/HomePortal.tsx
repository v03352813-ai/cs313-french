...
const TRACKS_CONFIG: Record<TrackId, TrackConfig> = {
  beginner: {
    ...
    tag: '系统筑基',
    icon: '🌱',
    activeBorder: 'border-[#243B5A]',
    activeBg: 'bg-[#FCFAF6] border-[#243B5A] text-[#243B5A]',
    activeRing: 'ring-2 ring-[#243B5A]/25 shadow-md',
    ...
    steps: [
      {
        ...
        badgeBg: 'bg-[#243B5A]/10 text-[#243B5A] border border-[#243B5A]/20',
        desc: '15 元音（含 4 鼻化音）、3 半元音与 17 辅音，动画透视联诵 (Liaison)、CaReFuL 词尾不发音与省音规则。',
        actionText: '进入音标联诵实验室',
        targetTab: 'phonetics',
        icon: Sparkles,
        buttonBg: 'bg-[#243B5A] hover:bg-[#1C2E46] text-white shadow-xs'
      },
      {
        stepNum: '02',
        ...
        badgeBg: 'bg-[#C5A059]/15 text-[#8F6F2E] border border-[#C5A059]/30',
        desc: '第一组(-er)、第二组(-ir)与第三组不规则动词，一键对比直陈式现在时、复合过去时、未完成过去时与虚拟式。',
        actionText: '开启变位演练器',
        targetTab: 'conjugation',
        icon: RotateCcw,
        buttonBg: 'bg-[#C5A059] hover:bg-[#B38E47] text-slate-900 font-black shadow-xs'
      },
      {
        stepNum: '03',
...
export const HomePortal: React.FC<HomePortalProps> = ({
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-4 sm:py-6 space-y-5 sm:space-y-6">
      ...
      {/* --- 1. 每日晨读打卡 & 持续更新动态跑马灯轮播专区 (正统法兰西皇家蓝风格 · 优雅白底旗舰卡片) --- */}
      <div id="daily-quote-section" className="grid grid-cols-1 lg:grid-cols-12 gap-3.5">
        ...
        <div className="lg:col-span-8 bg-[#FCFAF6] rounded-2xl p-4 sm:p-5 border border-[#E8DECE] shadow-xs flex flex-col justify-between space-y-3">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2 flex-wrap">
              <span className="p-1.5 rounded-lg bg-[#243B5A]/10 text-[#243B5A] font-bold text-xs flex items-center gap-1 border border-[#243B5A]/20">
            ...
            <div className="flex items-center gap-1.5">
              <button
                ...
                className="text-[11px] text-slate-500 hover:text-[#243B5A] flex items-center gap-1 px-2 py-1 rounded-lg hover:bg-[#F7F3EA] transition cursor-pointer font-bold"
                title="切换金句"
              >
          ...
          <div className="bg-[#F7F3EA] p-3.5 rounded-xl border border-[#E8DECE] flex items-center justify-between gap-3">
            ...
            <button
              ...
              className="p-2 rounded-full bg-white text-[#243B5A] hover:bg-[#FAF6EE] border border-[#E8DECE] shadow-2xs shrink-0 transition cursor-pointer"
              title="朗读金句"
            >
          ...
          <div className="flex items-center justify-between pt-0.5 text-xs">
            <div className="flex items-center gap-1.5 text-slate-600 text-[11px]">
              ...
              <span>已连续打卡 <strong className="text-[#243B5A] font-bold">{streak.count}</strong> 天</span>
            </div>
            ...
            <button
              ...
              className={`px-4 py-1.5 rounded-xl text-xs font-bold transition flex items-center gap-1 shadow-2xs cursor-pointer ${
                streak.isCheckedToday
                  ...
                  : 'bg-[#243B5A] hover:bg-[#1C2E46] text-white active:scale-95'
              }`}
            >
        ...
        >
              ...
              return (
                ...
                >
                  <div className="flex items-center justify-between gap-1.5">
                    ...
                    <span className="text-[#243B5A] font-bold text-[10px] bg-[#243B5A]/10 px-1.5 py-0.2 rounded border border-[#243B5A]/20 shrink-0">
                      {log.tag}
                    </span>
                  </div>
          ...
          {/* Footer note */}
          <div className="flex items-center justify-between pt-1 border-t border-[#E8DECE] text-[11px] text-slate-400">
              ...
              {CONTENT_UPDATE_LOGS.map((_, i) => (
                <span
                  ...
                  className={`w-1.5 h-1.5 rounded-full transition-all duration-300 ${
                    i === activeLogIndex ? 'bg-[#243B5A] w-3' : 'bg-slate-300'
                  }`}
                />
              ))}
            </div>
      ...
      {/* --- 2. 核心学习目标指引与 3 大主线选择器 (纯正法兰西巴黎深蓝视觉交互体系) --- */}
      <div className="bg-[#FCFAF6] rounded-3xl p-5 sm:p-6 border border-[#E8DECE] shadow-xs space-y-4">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2.5 pb-2">
          <div className="space-y-1">
            <div className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full bg-[#243B5A]/10 text-[#243B5A] text-xs font-bold border border-[#243B5A]/20">
              <Compass className="w-3.5 h-3.5 text-[#243B5A]" />
              <span>新学员指引 · 学习主线向导</span>
              <span className="text-slate-500 font-normal hidden sm:inline">不知道从哪学起？点击下方选定你的目标：</span>
            </div>
            <h2 className="text-xl sm:text-2xl font-black text-[#243B5A] tracking-tight">
              你当前的核心学习目标是什么？
            </h2>
          </div>

          <div className="flex items-center gap-1.5 self-start sm:self-auto px-3 py-1.5 rounded-xl bg-[#F7F3EA] border border-[#E8DECE] text-xs font-bold text-slate-700">
            <span className="text-slate-400 font-normal">当前主线:</span>
            <span className="text-[#243B5A] font-black">{currentTrackConfig.name}</span>
          </div>
        </div>

        {/* Selected Track Banner */}
        <div className="p-3.5 rounded-2xl bg-gradient-to-r from-[#243B5A]/10 via-[#8C3B4A]/5 to-[#F7F3EA] border border-[#E8DECE] flex flex-col sm:flex-row sm:items-center justify-between gap-2 text-xs">
          <div className="flex items-center gap-2">
            <span className="w-2 h-2 rounded-full bg-[#243B5A] shrink-0" />
          ...
          <span className="px-2.5 py-0.5 rounded-full bg-white text-[#243B5A] border border-[#E8DECE] text-[11px] font-bold self-start sm:self-auto shrink-0 shadow-2xs">
              ...
              <div
                ...
                className={`p-4 sm:p-5 rounded-2xl border transition-all duration-200 cursor-pointer relative flex flex-col justify-between space-y-3 group ${
                  isSelected
                    ...
                    : 'bg-[#FCFAF6] border-[#E8DECE] hover:border-[#C5A059]/60 hover:shadow-xs'
                }`}
              >
                {/* Active checkmark */}
                {isSelected && (
                  <div className="absolute top-3 right-3 w-5 h-5 rounded-full bg-[#243B5A] text-white flex items-center justify-center shadow-xs">
                    <CheckCircle2 className="w-3.5 h-3.5" />
                  </div>
                )}
                ...
                <div className="space-y-1.5">
                  <div className="flex items-center justify-between pr-6">
                    ...
                    <span className={`text-[10px] font-black px-2 py-0.5 rounded-full border ${
                      isSelected
                        ? 'bg-white text-slate-800 border-[#E8DECE]'
                        : 'bg-[#F7F3EA] text-slate-600 border-[#E8DECE]'
                    }`}>
                      {config.tag}
                    </span>
                  </div>

                  <h3 className="text-base font-black text-slate-900 group-hover:text-[#243B5A] transition">
                    {config.name}
                  </h3>
                ...
                <div className="pt-2 border-t border-[#E8DECE] flex items-center justify-between text-xs font-bold">
                  <span className={isSelected ? 'text-[#243B5A]' : 'text-slate-500 group-hover:text-slate-800'}>
                    {isSelected ? '立即进入学习 (进入真题大卷)' : '点击切换此路线'}
                  </span>
                  <ArrowRight className={`w-3.5 h-3.5 transition-transform group-hover:translate-x-0.5 ${
                    isSelected ? 'text-[#243B5A]' : 'text-slate-400'
                  }`} />
                </div>
              </div>
            );
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3">
            {currentTrackConfig.steps.map((step) => {
              ...
              return (
                <div
                  key={step.stepNum}
                  className="bg-[#F7F3EA] hover:bg-white rounded-2xl p-4 border border-[#E8DECE] hover:border-[#C5A059]/60 hover:shadow-xs transition flex flex-col justify-between space-y-3 group"
                >
                  <div className="space-y-2">
                    <div className="flex items-center justify-between">
                      <span className="text-[11px] font-black font-mono px-2 py-0.5 rounded-lg bg-[#FCFAF6] border border-[#E8DECE] text-[#243B5A]">
                        {step.stepLabel}
                      </span>
                    ...
                    <h4 className="text-sm font-black text-slate-900 group-hover:text-[#243B5A] transition">
                      {step.title}
                    </h4>
