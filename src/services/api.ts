/**
 * CS313 法语研习社 · 前端统一云端 API 数据服务层
 * 支持【Local-First 双模架构】：优先连云端 API，离线/弱网自动使用本地密码学防伪缓存兜底
 */
import type { LicenseInfo } from '../data/auth/cardKeys';
import { activateLicenseWithKey, saveLocalLicense, getLocalLicense } from '../data/auth/cardKeys';
...
class ApiService {
  ...
  async isServerOnline(): Promise<boolean> {
    ...
    try {
      const res = await fetch(`${API_BASE_URL}/admin/stats`, { method: 'GET', cache: 'no-cache' });
  ...
  async verifyCardKey(cardKey: string, device: DeviceInfo): Promise<{ success: boolean; message: string; license?: LicenseInfo; action?: string; cloudProgress?: any }> {
    try {
      ...
      if (data.success && data.license) {
        saveLocalLicense(data.license);
        // 关键：如果云端返回了此前备份的进度（换手机或清 Cookie 场景），立即恢复到本地！
        if (data.cloudProgress) {
          try {
            const localSaved = localStorage.getItem('cs313_fr_study_progress');
            ...
            localStorage.setItem('cs313_fr_study_progress', JSON.stringify(merged));
          } catch (e) {
    ...
    const localResult = activateLicenseWithKey(cardKey, device.deviceId, device.deviceType || '未知设备');
    if (localResult.success && localResult.license) {
      saveLocalLicense(localResult.license);
    }
    return localResult;
  }
  ...
  async syncStudyProgress(progress: Partial<StudentProgress>): Promise<StudentProgress> {
    const cachedLicense = getLocalLicense();
    let userId = 'std_local_guest';
    if (cachedLicense && cachedLicense.userId) {
      userId = cachedLicense.userId;
    }

    // 本地持久化缓存
    const localSavedStr = localStorage.getItem('cs313_fr_study_progress');
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (data.success && data.progress) {
          localStorage.setItem('cs313_fr_study_progress', JSON.stringify(data.progress));
  ...
  async getStudyProgress(userId: string): Promise<StudentProgress> {
    try {
      const res = await fetch(`${API_BASE_URL}/study/sync?userId=${encodeURIComponent(userId)}`);
      if (res.ok) {
        const data = await res.json();
        if (data.success && data.progress) {
          localStorage.setItem('cs313_fr_study_progress', JSON.stringify(data.progress));
   ...
   * 店主后台：一键批量生成卡密 (需口令鉴权)
   */
  async adminGenerateKeys(count: number = 10, tier: string = 'fr_lifetime'): Promise<any> {
    try {
