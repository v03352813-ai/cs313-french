...
const FR_SECRET_SALT = 'CS313_FR_2026_PRODUCTION_AUTH_KEY_V9X_TOP_SECRET';
...
const PRESET_VIP_KEYS = {
  ...
  'CS313-FR-8888-PARI': { tier: 'fr_lifetime', planName: '法语单语种终身VIP' },
  'CS313-FR-9999-LYON': { tier: 'fr_lifetime', planName: '法语单语种终身VIP' },
  'CS313-FR-5200-NICE': { tier: 'fr_lifetime', planName: '法语单语种终身VIP' },
  'CS313-JP-8888-HL3Y': { tier: 'jp_lifetime', planName: '日语单语种终身VIP' },
  'CS313-KR-8888-A1B2': { tier: 'kr_lifetime', planName: '韩语单语种终身VIP' },
};

function computeKeySignature(type, serial, salt = FR_SECRET_SALT) {
...
export default async function handler(req, res) {
  ...
  try {
    ...
    let tier = 'fr_lifetime';
    let planName = 'CS313 法语单语种终身VIP';
    ...
    if (PRESET_VIP_KEYS[cleanKey]) {
      ...
      // 3. 严格正则匹配标准格式: CS313-(FR|JP|KR|ALL)-[4位序号]-[4位签名]
      const match = cleanKey.match(/^CS313-(FR|JP|KR|ALL)-([0-9A-Z]{4})-([0-9A-Z]{4})$/);
      if (!match) {
        await recordFailedAttempt(clientIp);
        return res.status(400).json({
          success: false,
          message: '激活码格式无效（标准格式示例：CS313-FR-8888-PARI 或 CS313-ALL-VIP8-87GT）。'
        });
      }
      ...
      // 4. 密码学数学签名校验（同时适配 FR、JP 与 KR 盐）
      const expectedSigFR = computeKeySignature(type, serial, FR_SECRET_SALT);
      ...
      if (providedSig !== expectedSigFR && providedSig !== expectedSigJP && providedSig !== expectedSigKR) {
        await recordFailedAttempt(clientIp);
        return res.status(400).json({
          success: false,
      ...
      tier = type === 'ALL' ? 'all_lang' : (type === 'FR' ? 'fr_lifetime' : (type === 'JP' ? 'jp_lifetime' : 'kr_lifetime'));
      planName = type === 'ALL' ? '全球小语种黑金终身通卡' : (type === 'FR' ? '法语单语种终身VIP' : (type === 'JP' ? '日语单语种终身VIP' : '韩语单语种终身VIP'));
    }
    ...
    // 6. 检查并拉取学员此前同步在云端的学习数据（用于换手机或清 Cookie 后恢复做题本）
