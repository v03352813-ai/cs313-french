/**
 * CS313 法语研习社 · 云端数据库统一适配器 (api/lib/db.js)
 * 
 * 兼容支持：
...
export async function bindDeviceToLicense(cardKey, device, tier) {
  ...
  if (!existing) {
    ...
    return {
      ...
      license: newLicense,
      message: '🎉 恭喜！卡密首次验证激活成功，已绑定当前设备为终身授权终端。'
    };
  }
  ...
  if (existing.status === 'SUSPENDED') {
    return {
      success: false,
      message: '❌ 该卡密因异常频繁跨地域请求已被系统安全挂起，请联系店主客服微信解封。'
    };
  }

  const boundDevices = existing.boundDevices || [];
  const isAlreadyBound = boundDevices.some(d => d.id === deviceId);

  // B-1: 当前设备正是原绑定设备之一（换浏览器、清理 Cookie、再次登录场景）
  if (isAlreadyBound) {
    existing.lastActiveAt = now;
    await saveLicense(cleanKey, existing);
    return {
      success: true,
      action: 'DEVICE_RE_LOGIN',
      license: existing,
      message: '✨ 欢迎回来！已识别当前授权设备，终身 VIP 学习权益已立即恢复。'
    };
  }

  // B-2: 新设备绑定，但未超过最大限制（如：此前仅绑定了手机，现在新绑定电脑）
  const maxLimit = existing.maxDevices || 2;
  if (boundDevices.length < maxLimit) {
    boundDevices.push({
      id: deviceId,
      name: deviceName,
      boundAt: now
    });
    existing.boundDevices = boundDevices;
    existing.lastActiveAt = now;
    await saveLicense(cleanKey, existing);
    return {
      success: true,
      action: 'DEVICE_ADDED',
      license: existing,
      message: `🎉 绑定成功！已成功添加第 ${boundDevices.length}/${maxLimit} 台学习授权设备。`
    };
  }

  // B-3: 超过设备上限拦截（严格防盗刷防分享）
  return {
    success: false,
    action: 'DEVICE_LIMIT_EXCEEDED',
    boundDevicesCount: boundDevices.length,
    maxDevices: maxLimit,
    message: `⚠️ 激活受限：该卡密已绑定满 ${maxLimit} 台设备（如手机与iPad）。根据终身授权防盗刷规范，不可再绑定新设备。如需解绑请联系店主客服。`
  };
}

/**
 * 4. 学员学习进度云端持久化存储 (Mastered 单词、打卡天数等)
 */
